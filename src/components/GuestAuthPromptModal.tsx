...
export const GuestAuthPromptModal: React.FC<GuestAuthPromptModalProps> = ({
  actionMessage = 'Para ler, descarregar e interagir com as obras da Ala-X, é necessário ter uma conta activa.',
  onClose,
  onOpenAuth,
}) => {
  return (
      ...
      >
        ...
        <div className="space-y-2 pt-2">
          <button
            ...
            className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 via-amber-400 to-amber-300 text-black font-black text-xs shadow-xl shadow-amber-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            <LogIn className="w-4 h-4" />
            <span>Iniciar Sessão / Criar Conta Grátis</span>
          </button>

          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-2xl bg-transparent hover:bg-white/5 text-slate-400 text-xs font-bold transition-all cursor-pointer"
          >
